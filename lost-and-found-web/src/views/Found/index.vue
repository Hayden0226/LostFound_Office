<template>
  <div>
    <h2 style="margin-bottom:20px">浏览失物招领</h2>
    <el-row :gutter="20">
      <el-col :span="6">
        <el-input v-model="keyword" placeholder="搜索失物招领" clearable @change="search" />
      </el-col>
      <el-col :span="4">
    ...
    <el-row :gutter="20" style="margin-top:20px">
      <el-col :span="8" v-for="item in list" :key="item.id" style="margin-bottom:20px">
        <el-card shadow="hover" @click="$router.push('/home/found/' + item.id)" style="cursor:pointer">
          ...
          <el-tag v-if="item.category" size="small" style="margin-bottom:5px">{{ item.category }}</el-tag>
          <p style="color:#666;font-size:12px">{{ item.location }} 路 {{ item.createTime }}</p>
...
async function loadData() {
  const res = await getFoundList({ page: page.value, size: size.value, keyword: keyword.value, category: category.value })
